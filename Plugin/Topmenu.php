...
class Topmenu
{
    ...
    public function beforeGetHtml(
        ...
        $customText = $this->scopeConfig->getValue(
            'nav_custom/nav_button/text',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );

        $customIcon = $this->scopeConfig->getValue(
            'nav_custom/nav_button/icon',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        ...
        $iconClass = $customIcon && $customIcon !== 'none' ? 'icon ' . $customIcon : '';

        // Add Custom Menu
        $node = $this->nodeFactory->create([
            'data' => [
                'name' => $customText ?? __('Le Club'),
                'class' => 'contestio-link ' . $activeClass . ' ' . $iconClass . ' ' . $outermostClass,
                'url' => $this->_urlBuilder->getUrl(null, ['_direct' =>'contestio/accueil']),
                'has_active' => false,
                'is_active' => false
            ],
            'idField' => 'id',
